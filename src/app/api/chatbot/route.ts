...
async function getGeminiResponse(message: string, apiKey: string): Promise<{response: string, suggestions?: string[]}> {
  try {
...
function findRelevantSuggestions(userMessage: string, aiResponse: string, faqData: FAQData): string[] {
  ...
  for (const faq of faqData.faqs) {
    ...
    Object.entries(topicKeywords).forEach(([topic, keywords]) => {
      const messageHasKeywords = keywords.some(keyword => 
        lowerMessage.includes(keyword) || lowerResponse.includes(keyword)
      );
      const faqHasKeywords = keywords.some(keyword => 
...
async function getFallbackResponse(message: string): Promise<{response: string, suggestions?: string[]}> {
  ...
  for (const faq of (faqData as FAQData).faqs) {
    ...
    Object.entries(keywordMap).forEach(([topic, keywords]) => {
      ...
      const topicInFAQ = keywords.some(keyword => 
        faq.question.toLowerCase().includes(keyword) || 
        faq.answer.toLowerCase().includes(keyword)
      );
  ...
  if (lowerMessage.includes('hello') || lowerMessage.includes('hi') || lowerMessage.includes('hey')) {
    return {
      response: "Hello! 👋 I'm Ronit's AI assistant, here to help you learn about his work in geospatial development and software engineering. Feel free to ask about his projects, skills, experience at Camptocamp, or anything else related to his professional background!",
      suggestions: [
        "What is Ronit's current role and company?",
  ...
  if (lowerMessage.includes('help') || lowerMessage.includes('what can') || lowerMessage.includes('tell me')) {
    return {
      response: "I can help you learn about Ronit Jadhav! Ask me about: 🗺️ His geospatial expertise (QGIS, OpenLayers, GIS) 💻 Programming skills (Python, JavaScript, TypeScript) 🏢 Current role at Camptocamp 🚀 Projects like Digipin, QGIS Hub Plugin 🎓 Education and career journey 📧 How to get in touch. What interests you most?",
      suggestions: [
        "Who is Ronit Jadhav?",
  ...
  return {
    response: "I'm here to share information about Ronit Jadhav's professional background! You can ask me about his work at Camptocamp, his expertise in geospatial technologies, specific projects he's worked on, his education, or how to contact him. What would you like to know? 🤔",
    suggestions: [
      "Who is Ronit Jadhav?",
      "What is Ronit's current role and company?",
      "What technologies does he work with?"
