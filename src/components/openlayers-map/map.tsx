...
const TimelineItem: React.FC<TimelineItemProps> = ({ entry, onClick }) => (
  <div
    onClick={onClick}
    className="cursor-pointer w-full transition-all duration-300 hover:bg-yellow-100 dark:hover:bg-gray-500"
  >
...
const TimelineContainer: React.FC<TimelineContainerProps> = ({ isOpen, onClose, children }) => (
  ...
  >
    ...
    <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar relative">{children}</div>
  </div>
);
...
const MapComponent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    mapInstance.on('click', (event) => {
      ...
      if (feature) {
        ...
        popupRoot.render(<Popup title={props.popupTitle} description={props.popupDescription} />);
  ...
  return (
    <div className="flex flex-col gap-8 p-8 dark:bg-darkBg">
      ...
      <div
        className="relative border-4 border-black dark:darkBorder bg-white dark:bg-darkBg
